...
export const receiveCalendar = (
  month: string,
  data: ICalendarDataProviderResponse,
) => ({
  data,
  month,
...
const fetchCalendar = (month: string) => async (dispatch: any) => {
  ...
  try {
    // @ts-ignore
    const response = await fetch(`${URL}/calendar?month=${month}`, {
...
export const dispatchFetchCalendar = (month: string) => (dispatch: any) =>
  dispatch(fetchCalendar(month));

export const dispatchRefetchCalendar = () => (
  dispatch: any,
